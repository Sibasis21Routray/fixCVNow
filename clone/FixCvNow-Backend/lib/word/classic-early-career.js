// lib/word/classic-early-career.js
// Word — Classic Early Career (template 6)
// Identical to Classic Professional except "Profile Summary" → "Career Objective"
...
export function buildClassicEarlyCareerDoc(data) {
  ...
  if (data.summary) {
    paragraphs.push(sectionHeader('Career Objective'))
  ...
  if (data.education?.length > 0) {
    ...
    const headerRow = new TableRow({
      children: [
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: 'Course', bold: true, color: CW.accentText, size: SZ.muted, font: T.font.word })] })], shading: { fill: CW.accent, type: ShadingType.CLEAR, color: CW.accent }, width: { size: EDU_C1, type: WidthType.DXA }, borders: cellBorders() }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: 'University / Board', bold: true, color: CW.accentText, size: SZ.muted, font: T.font.word })] })], shading: { fill: CW.accent, type: ShadingType.CLEAR, color: CW.accent }, width: { size: EDU_C2, type: WidthType.DXA }, borders: cellBorders() }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: 'Year', bold: true, color: CW.accentText, size: SZ.muted, font: T.font.word })] })], shading: { fill: CW.accent, type: ShadingType.CLEAR, color: CW.accent }, width: { size: EDU_C3, type: WidthType.DXA }, borders: cellBorders() }),
        new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: '%/Grade', bold: true, color: CW.accentText, size: SZ.muted, font: T.font.word })] })], shading: { fill: CW.accent, type: ShadingType.CLEAR, color: CW.accent }, width: { size: EDU_C4, type: WidthType.DXA }, borders: cellBorders() }),
      ],
    })
    const dataRows = data.education.map((edu, idx) => new TableRow({
      children: [
        new TableCell({ width: { size: EDU_C1, type: WidthType.DXA }, children: [new Paragraph({ children: [new TextRun({ text: `${edu.degree || ''}${(edu.fieldOfStudy || edu.field) ? ` (${edu.fieldOfStudy || edu.field})` : ''}`, size: SZ.muted, font: T.font.word, color: CW.textBody })] })], shading: idx % 2 === 1 ? { fill: 'F9FAFB', type: ShadingType.CLEAR } : {}, borders: cellBorders() }),
        new TableCell({ width: { size: EDU_C2, type: WidthType.DXA }, children: [new Paragraph({ children: [new TextRun({ text: edu.institution || edu.school || '', size: SZ.muted, font: T.font.word, color: CW.textBody })] })], shading: idx % 2 === 1 ? { fill: 'F9FAFB', type: ShadingType.CLEAR } : {}, borders: cellBorders() }),
        new TableCell({ width: { size: EDU_C3, type: WidthType.DXA }, children: [new Paragraph({ children: [new TextRun({ text: edu.endYear || edu.end || edu.year || '', size: SZ.muted, font: T.font.word, color: CW.textMuted })] })], shading: idx % 2 === 1 ? { fill: 'F9FAFB', type: ShadingType.CLEAR } : {}, borders: cellBorders() }),
        new TableCell({ width: { size: EDU_C4, type: WidthType.DXA }, children: [new Paragraph({ children: [new TextRun({ text: edu.grade || edu.percentage || edu.gpa || '-', size: SZ.muted, font: T.font.word, color: CW.textMuted })] })], shading: idx % 2 === 1 ? { fill: 'F9FAFB', type: ShadingType.CLEAR } : {}, borders: cellBorders() }),
      ],
    }))
    paragraphs.push(new Table({ width: { size: TEXT_W, type: WidthType.DXA }, layout: TableLayoutType.FIXED, columnWidths: [EDU_C1, EDU_C2, EDU_C3, EDU_C4], rows: [headerRow, ...dataRows] }))
  }
  ...
  if (allSkills.length > 0) {
    ...
    for (let i = 0; i < allSkills.length; i += 2) {
      skillRows.push(new TableRow({
        children: [
          new TableCell({ children: [bullet(allSkills[i])], width: { size: HALF, type: WidthType.DXA }, borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
          new TableCell({ children: [allSkills[i + 1] ? bullet(allSkills[i + 1]) : new Paragraph('')], width: { size: HALF, type: WidthType.DXA }, borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
        ],
      }))
    }
  ...
  if (data.languages?.length > 0) {
    ...
    for (let i = 0; i < data.languages.length; i += 2) {
      langRows.push(new TableRow({
        children: [
          new TableCell({ children: [bullet(getLangText(data.languages[i]))], width: { size: HALF, type: WidthType.DXA }, borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
          new TableCell({ children: [data.languages[i + 1] ? bullet(getLangText(data.languages[i + 1])) : new Paragraph('')], width: { size: HALF, type: WidthType.DXA }, borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
        ],
      }))
    }
  ...
  if (data.certifications?.length > 0) {
    ...
    for (const cert of data.certifications) {
      paragraphs.push(new Paragraph({ children: [new TextRun({ text: cert.name, bold: true, size: SZ.body, color: CW.textPrimary, font: T.font.word })], spacing: { after: 40 } }))
      const certOrg = cert.issuingOrganization || cert.issuer
      const certDate = cert.issueDate || cert.year
      if (certOrg || certDate) {
        paragraphs.push(new Paragraph({ children: [new TextRun({ text: [certOrg, certDate].filter(Boolean).join(' | '), size: SZ.muted, color: CW.textMuted, font: T.font.word })], spacing: { after: 60 } }))
      }
    }
  }
  ...
  if (data.interests?.length > 0) {
    ...
    for (let i = 0; i < data.interests.length; i += 2) {
      intRows.push(new TableRow({
        children: [
          new TableCell({ children: [bullet(data.interests[i])], width: { size: HALF, type: WidthType.DXA }, borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
          new TableCell({ children: [data.interests[i + 1] ? bullet(data.interests[i + 1]) : new Paragraph('')], width: { size: HALF, type: WidthType.DXA }, borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
        ],
      }))
    }
    if (intRows.length > 0) {
